...
import { useDeleteRestockRequestMutation } from "@/redux/features/restockRequest/restockRequest.api";
import { IQueruMutationErrorResponse } from "@/types";
import { useState } from "react";
import { FaRegTrashAlt } from "react-icons/fa";
import { toast } from "sonner";

const DeleteRestockRequestModal = ({
  restockRequestId,
  restockRequestName,
}: {
  restockRequestId: string;
  restockRequestName: string;
}) => {
  const [isOpen, setIsOpen] = useState(false);
  const [inputValue, setInputValue] = useState("");

  const [deleteRestockRequest, { isLoading }] = useDeleteRestockRequestMutation();

  const handleDelete = async () => {
    if (inputValue !== "DELETE") return;
    const res = await deleteRestockRequest(restockRequestId);
    ...
    toast.success("Restock request deleted successfully");
    setIsOpen(false);
  };
        ...
        <div className="relative w-full max-w-md transform overflow-hidden rounded-lg bg-white px-6 py-4 shadow-xl transition-all">
          ...
          <div>
            ...
            <div className="mt-3 text-center">
              <h3 className="text-lg leading-6 font-medium text-primary">Delete Restock Request</h3>
              <div className="mt-2">
...
export default DeleteRestockRequestModal;
