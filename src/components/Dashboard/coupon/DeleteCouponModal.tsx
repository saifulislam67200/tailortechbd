...
const DeleteCouponModal = ({ coupon }: { coupon: ICoupon }) => {
  const [deleteCoupon, { isLoading }] = useDeleteCouponMutation();
  const [isOpen, setIsOpen] = useState(false);
  const [confirmText, setConfirmText] = useState("");
  const inputRef = useRef<HTMLInputElement>(null);

  const isConfirmValid = confirmText === coupon.code;
  ...
  return (
    <>
      <button
        ...
        className="flex h-[35px] w-[35px] cursor-pointer items-center justify-center rounded-full bg-danger/10 p-[4px] text-[15px] text-danger"
      >
        <FaRegTrashAlt />
      </button>
      <DialogProvider state={isOpen} setState={setIsOpen}>
        <div className="mx-4 w-full max-w-md overflow-hidden rounded-lg bg-white shadow-xl">
          ...
          <div className="border-b border-gray-200 p-6">
            <h3 className="text-xl font-semibold text-danger">Delete Coupon</h3>
            <p className="mt-2 text-sm text-primary">
              This action cannot be undone. This will soft delete the coupon.
            </p>
          </div>
          ...
          <div className="space-y-4 p-6">
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-primary">Coupon Name</label>
            ...
            <div className="space-y-2">
              <label htmlFor="confirm" className="block text-sm font-medium text-primary">
                Type <span className="font-mono font-bold text-danger">{coupon.code}</span> to
                confirm
              </label>
              <input
                ref={inputRef}
            ...
            {confirmText && !isConfirmValid && (
              <p className="text-sm text-danger">
                Please type {`"${coupon.discountType}"`} exactly to confirm
              </p>
            )}
          </div>
